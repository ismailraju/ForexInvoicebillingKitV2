...
//        });
    ...
    public void handle(KeyEvent event) {
        if (event.getCode() == KeyCode.DOWN || event.getCode() == KeyCode.UP || event.getCode() == KeyCode.TAB) {
            ...
            if ((((System.nanoTime() - startTime) / 1000000) > 1000) && sb.length() > 0) {
                sb.delete(0, sb.length());
            }
        ...
        for (int i = 0; i < items.size(); i++) {
            ...
            if (event.getCode() != KeyCode.BACK_SPACE && items.get(i).toString().toLowerCase().startsWith(sb.toString().toLowerCase())) {
                ...
                k = i;
                break;
            } else if (sb.length() > 2 && (sb.charAt(sb.length() - 2) == sb.charAt(sb.length() - 1))) {
                System.out.println(sb.charAt(sb.length() - 2));
                ListView lv = ((ComboBoxListViewSkin) comboBox.getSkin()).getListView();
                //lv.setPrefSize(200, 200);
                String s = String.valueOf(sb.charAt(sb.length() - 1));
                if (!items.get(j).toString().toLowerCase().startsWith(s.toLowerCase())) {
                    j = k;
                   // System.out.println(s + ": not matching");
                }
                lv.getSelectionModel().clearAndSelect(j++);
                lv.scrollTo(lv.getSelectionModel().getSelectedIndex());
                break;
            }
        }
